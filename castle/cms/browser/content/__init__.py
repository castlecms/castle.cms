...
class Creator(BrowserView):
    ...
    def detect_duplicate(self, info):
        ...
        sha256_encrypt = commands.sha256(info['tmp_file'])
        obj = dup_detector.get_object(sha256_encrypt)
        if obj is not None:
            # found, use existing file...
            raise duplicates.DuplicateException(obj)
        return dup_detector, sha256_encrypt
    ...
    def create_file_content(self, info):
        ...
        sha256_encrypt = dup_detector = None
        ...
        if original_location == location:
            ...
            if type_ == 'Image' and commands.sha256:
                dup_detector, sha256_encrypt = self.detect_duplicate(info)
            self.handle_auto_folder_creation(folder, type_)

        obj = self.create_object(folder, type_, info)
        if type_ == 'Image' and sha256_encrypt and dup_detector:
            dup_detector.register(obj, sha256_encrypt)
        return obj
