...
from castle.cms.fragments import (
    FragmentsDirectory,
    ThemeFragmentsDirectory,
    FileChangedCacheFactory,
    FileCacheFactory,
    FragmentView)
...
class FragmentsCacheTesting(unittest.TestCase):
    ...
    def test_FileCache_cache(self):
        ...
        for filepath in cache:
            ...
            if cache[filepath]['template'] is None:
                self.assertTrue(cache[filepath]['mtime'] == 0)
            else:
                self.assertTrue(isinstance(cache[filepath]['template'], ZopePageTemplate))
                self.assertTrue(type(cache[filepath]['mtime']) == int or
                                type(cache[filepath]['mtime']) == float)
        ...
        self.assertTrue(result is None)
        cache = filecache.get_cache_storage()
        self.assertTrue(cache[false_template_path]['template'] is None)
        self.assertTrue(cache[false_template_path]['mtime'] == 0)

    def test_FileChangedCache_cache(self):
        # Tests the FileChangedCacheFactory get functionality and
        # check to see that the cache structure is correct
        ...
        for filepath in cache:
            self.assertTrue(type(filepath) == str)
            if cache[filepath]['template'] is None:
                self.assertTrue(cache[filepath]['mtime'] == 9999999999 or
                                cache[filepath]['mtime'] == 0)
            else:
                self.assertTrue(isinstance(cache[filepath]['template'], ZopePageTemplate))
