...
class Forum(IConversation):
    def __call__(self, title, description, created, topics):
        self.title = title
        self.description = description
        self.created = created
        self.topics = topics
class Comment(IComment):
    def __call__(self, author_username, text, creation_date):
        self.author_username = author_username
        self.text = text
        self.creation_date = creation_date
        self.creator = api.user.get(username=author_username).fullname
    ...
    for community_name in os.listdir(path):
        ...
        if os.path.exists(wiki_folder_path):
            ...
            for wiki_page in os.listdir(wiki_folder_path):
                try:
                    if api.content.get(os.path.join(comm_wiki_sitepath, wiki_page)) is None:
